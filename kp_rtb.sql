...
CREATE FUNCTION target_detection() RETURNS trigger
    AS '
        DECLARE
            ...
            change integer := 0;
        BEGIN
            RAISE NOTICE ''Вставляю %'', NEW.p;
            SELECT RLS.change INTO change FROM RLS WHERE RLS.id=NEW.rls_id;
            RAISE NOTICE ''Смена %'', change;
            IF change <> 1 THEN
                RAISE EXCEPTION ''Не та смена'';
            END IF;
            RETURN NEW;
        END;
    '
...
\echo Вывести принадлежащие текущему КП радиотехнического батальона РЛС с расстояниями до них
...
\echo Вывести обнаруженные в данный момент цели с расстояниями до РЛС
SELECT
    RLS.p AS rls_p,
...
\echo Вывести для каждой цели ближайшую РЛС в данный момент времени
